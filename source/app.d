...
bool loadLib()
{
    ...
    if(ret != sdlSupport)
    {
        writeln("ret does not say sdlSupport");
        //Log the error info
        ...
        else
        {
			...
			//msg = "Your SDL version is too low: "~ itoa(version.major)~"."~
			//	itoa(version.minor)~"."~
			//	itoa(version.patch)~
...
int main() 
{
    ...
    else
    {
        ...
        if( sdlWindow == null )
        {
            printf( "sdlWindow could not be created! SDL_Error: %s\n", SDL_GetError() );
        }	
        else
        {
            //Get window surface
